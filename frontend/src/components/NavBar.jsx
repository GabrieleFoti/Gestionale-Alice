    ...
    <nav className="sticky top-0 z-50 border-b bg-brand-bg-300 border-brand-bg-700">
        ...
        <div className="flex justify-between h-16">
          <div className="flex">
            <div className="flex flex-shrink-0 items-center">
              <span className="font-[nighty] text-2xl sm:text-5xl">Panzani Design</span>
            </div>
            ...
            {isAuthenticated && isAdmin && (
              <div className="hidden sm:ml-8 sm:flex sm:space-x-8">
                <Link
                  to="/officina"
                  className={`inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium transition-colors ${location.pathname === '/officina'
                ...
                <Link
                  to="/archivio"
                  className={`inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium transition-colors ${location.pathname === '/archivio'
                      ? 'border-brand-text-700 text-brand-text'
                      : 'border-transparent text-brand-text/70 hover:border-brand-text hover:text-brand-text'
                    }`}
                >
      ...
      {isAuthenticated && isAdmin && (
        <div className="flex overflow-x-auto px-4 py-2 space-x-4 border-t border-brand-text-200 sm:hidden">
          <Link
            to="/officina"
            className={`text-sm font-medium ${location.pathname === '/officina' ? 'text-brand-text underline' : 'text-brand-text/70'
              }`}
          >
            Officina
          </Link>
          <Link
            to="/archivio"
            className={`text-sm font-medium ${location.pathname === '/archivio' ? 'text-brand-text underline' : 'text-brand-text/70'
              }`}
          >
            Archivio
          </Link>
